...
fun ResetMpinScreen(
    ...
    LaunchedEffect(bothFilled) {
        if (bothFilled) viewModel.resetMpin(mobile, otp)
    }
    ...
    Box(
        ...
        Column(
            ...
            Text(str.mpinResetTitle, style = MaterialTheme.typography.headlineMedium,
                color = Color.White, fontWeight = FontWeight.Bold)
            Spacer(Modifier.height(4.dp))
            Text(str.mpinResetSubtitle,
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp), verticalAlignment = Alignment.CenterVertically) {
                repeat(2) { i ->
                    val active = i + 1 == (if (!enteringConfirm) 1 else 2)
                    val done   = i + 1 < (if (!enteringConfirm) 1 else 2)
            ...
            Text(
                if (!enteringConfirm) "Enter new MPIN" else "Confirm new MPIN",
