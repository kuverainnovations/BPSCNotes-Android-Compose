        ...
        Column(
            ...
            Row(
                ...
                Column(horizontalAlignment = Alignment.CenterHorizontally) {
                    Text("Study Session", style = MaterialTheme.typography.titleMedium, color = Color.White, fontWeight = FontWeight.Bold)
                    if (state.tierName != null) {
                        Text("${state.tierEmoji ?: ""} ${state.tierName}", style = MaterialTheme.typography.bodyMedium, color = Color.White.copy(0.7f))
                    }
                }
                // Mode badge
                Box(modifier = Modifier.clip(RoundedCornerShape(20.dp)).background(Color.White.copy(0.15f)).padding(horizontal = 10.dp, vertical = 5.dp)) {
                    Text(state.mode.replaceFirstChar { it.uppercase() }, style = MaterialTheme.typography.labelSmall, color = Color.White, fontWeight = FontWeight.Bold)
                }
            }
                ...
                Card(
                    ...
                    Row(modifier = Modifier.fillMaxWidth().padding(14.dp), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
                        Row(horizontalArrangement = Arrangement.spacedBy(10.dp), verticalAlignment = Alignment.CenterVertically) {
                            ...
                            Column {
                                Text("AFK Detected", style = MaterialTheme.typography.titleMedium, color = Color.White, fontWeight = FontWeight.Bold)
                                Text("Study time was not counted for the idle period.", style = MaterialTheme.typography.bodySmall, color = Color.White.copy(0.85f))
                            }
                        }
            ...
            Box(modifier = Modifier.size(220.dp), contentAlignment = Alignment.Center) {
                ...
                Column(horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(4.dp)) {
                    Text(timeStr, style = MaterialTheme.typography.headlineLarge, color = Color.White, fontWeight = FontWeight.ExtraBold, fontSize = 36.sp)
                    Text("active", style = MaterialTheme.typography.bodyMedium, color = Color.White.copy(0.65f))
                    if (state.wasAfkLastBeat) {
                        Text("⚠️ AFK period", style = MaterialTheme.typography.labelSmall, color = Color(0xFFFF6B35))
                    } else {
                        Text("🟢 Active", style = MaterialTheme.typography.labelSmall, color = BpscColors.Success)
                    }
                }
            }
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(20.dp)) {
                SessionStatChip(label = "Coins", value = "+${state.coinsThisSession}", icon = "🪙")
            ...
            if (state.coinsLastBeat > 0 || state.xpLastBeat > 0) {
                ...
                Row(modifier = Modifier.clip(RoundedCornerShape(20.dp)).background(Color.White.copy(0.12f)).padding(horizontal = 14.dp, vertical = 7.dp), horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                    if (state.coinsLastBeat > 0) Text("🪙+${state.coinsLastBeat}", style = MaterialTheme.typography.bodyMedium, color = BpscColors.CoinGold, fontWeight = FontWeight.Bold)
                    if (state.xpLastBeat > 0) Text("⚡+${state.xpLastBeat} XP", style = MaterialTheme.typography.bodyMedium, color = Color(0xFF64B5F6), fontWeight = FontWeight.Bold)
                }
            }
            ...
            Button(
                ...
                colors   = ButtonDefaults.buttonColors(containerColor = Color.White.copy(0.15f), contentColor = Color.White)
            ) {
                if (state.status == SessionStatus.ENDING) {
                    CircularProgressIndicator(color = Color.White, modifier = Modifier.size(20.dp), strokeWidth = 2.dp)
                } else {
                    Icon(Icons.Rounded.StopCircle, null, modifier = Modifier.size(20.dp)); Spacer(Modifier.width(8.dp))
...
@Composable
private fun SessionStatChip(label: String, value: String, icon: String) {
    Column(horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(4.dp)) {
        Text(icon, fontSize = 20.sp)
        Text(value, style = MaterialTheme.typography.titleLarge, color = Color.White, fontWeight = FontWeight.ExtraBold)
        Text(label, style = MaterialTheme.typography.labelSmall, color = Color.White.copy(0.65f), fontSize = 10.sp)
    }
}
...
private fun SessionSummarySheet(
    ...
    Box(
        modifier = Modifier.fillMaxSize().background(
            Brush.verticalGradient(listOf(Color(0xFF0A2472), Color(0xFF1565C0), BpscColors.Surface))
        ),
        ...
        Column(
            ...
            Text(summary?.message ?: "Great work!", style = MaterialTheme.typography.bodyLarge, color = Color.White.copy(0.75f), textAlign = TextAlign.Center, modifier = Modifier.padding(horizontal = 32.dp))
            ...
            Box(Modifier.size(120.dp), contentAlignment = Alignment.Center) {
                androidx.compose.foundation.Canvas(Modifier.fillMaxSize()) {
                    ...
                    val sz     = androidx.compose.ui.geometry.Size(size.width - stroke, size.height - stroke)
                    drawArc(Color.White.copy(0.15f), -90f, 360f, false, style = Stroke(stroke), topLeft = Offset(inset, inset), size = sz)
                    drawArc(Brush.sweepGradient(listOf(Color(0xFF64B5F6), Color.White)), -90f, animProg * 360f, false, style = Stroke(stroke, cap = StrokeCap.Round), topLeft = Offset(inset, inset), size = sz)
                }
                Column(horizontalAlignment = Alignment.CenterHorizontally) {
                    Text("${summary?.activeMinutes ?: 0}m", style = MaterialTheme.typography.headlineMedium, color = Color.White, fontWeight = FontWeight.ExtraBold)
                    Text("active", style = MaterialTheme.typography.labelSmall, color = Color.White.copy(0.7f))
                }
            }
            ...
            if ((summary?.bonusCoins ?: 0) > 0) {
                ...
                Card(modifier = Modifier.fillMaxWidth().padding(horizontal = 24.dp), shape = RoundedCornerShape(14.dp), colors = CardDefaults.cardColors(containerColor = BpscColors.CoinGold.copy(0.15f)), elevation = CardDefaults.cardElevation(0.dp)) {
                    Row(modifier = Modifier.fillMaxWidth().padding(14.dp), horizontalArrangement = Arrangement.spacedBy(10.dp), verticalAlignment = Alignment.CenterVertically) {
                        Text("🎉", fontSize = 20.sp)
                        Text("Bonus +${summary?.bonusCoins} coins for 30+ active minutes!", style = MaterialTheme.typography.bodyMedium, color = BpscColors.TextPrimary, fontWeight = FontWeight.SemiBold)
                    }
                }
            }
...
@Composable
private fun SumItem(icon: String, value: String, label: String) {
    Column(horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(4.dp)) {
        Text(icon, fontSize = 20.sp)
